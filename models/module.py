...
class ConvAttention_conv5(nn.Module):
    def __init__(self, dim, img_size, heads = 8, dim_head = 64, kernel_size=3, q_stride=1, k_stride=1, v_stride=1, dropout = 0.,
        ...
        self.to_q = SepConv3d(dim, inner_dim, kernel_size, q_stride, pad) # 3D Conv BatchNorm Conv 
        self.to_k = SepConv3d(dim, inner_dim, kernel_size, k_stride, pad) # 3D Conv BatchNorm Conv 
        self.to_v = SepConv3d(dim, inner_dim, kernel_size, v_stride, pad) # 3D Conv BatchNorm Conv 
    ...
    def forward(self, x):
        ...
        # print(x.shape) # b 64(채널수) 8 16 16 
        x = rearrange(x, 'b (z l w) n -> b n z l w', z=4, l=5, w=10) 
        q = self.to_q(x)
...
class ConvAttention_conv4(nn.Module):
    ...
    def forward(self, x):
        ...
        # # what's x shape?  Input x sahpe의 영향이 중요하다.
        # Additional Information
        
        # print(x.shape) # b 64(채널수) 8 16 16 
        x = rearrange(x, 'b (z l w) n -> b n z l w', z=8, l=10, w=20) 
        q = self.to_q(x)
        q = rearrange(q, 'b (h d) z l w -> b h (z l w) d', h=h) 
